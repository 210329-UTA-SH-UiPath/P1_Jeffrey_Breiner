...
namespace PizzaBox.Storing.Repositories
{
    public class RepositoryTopping : IRepository<ATopping>
    {
        ...
        {
            return context.DBToppings.AsNoTracking().Select(mapperTopping.Map).ToList();
        }

        public ATopping GetById(int id)
        {
            var dbTopping = context.DBToppings.AsNoTracking().FirstOrDefault(topping => topping.ID == id);
        ...
        {
            DBTopping topping = context.DBToppings.ToList().Find(topping => topping.ID == id);
